...
class BST(basemodel.BaseRanker):
    r"""
    ...
    def _get_scorer(self, train_data):
        return module.ctr.BehaviorSequenceTransformer(
            ...
            train_data.config['max_seq_len'], self.embed_dim, self.config['hidden_size'],
            self.config['num_layer'], self.config['num_head'], dropout=self.config['dropout'],
            mlp_layers=self.config['mlp_layer'], activation=self.config['activation'], norm_first=False
        )
