...
class CaserQueryEncoder(torch.nn.Module):
    ...
    def __init__(self, fiid, fuid, num_users, num_items, embed_dim, max_seq_len, n_v, n_h, dropout=0.2) -> None:
        ...
        self.fc_w = torch.nn.Linear(n_v*embed_dim+n_h*max_seq_len, embed_dim, bias=True)
    ...
    def forward(self, batch):
        ...
        o = self.dropout(o)
        z = torch.relu(self.fc_w(o))
        ones_bias = z.new_ones((z.size(0), 1))
        return torch.cat((z, P_u, ones_bias), dim=1)
...
class Caser(basemodel.BaseRetriever):
    ...
    def _get_item_encoder(self, train_data):
        ...
        """
        return torch.nn.Embedding(train_data.num_items, self.embed_dim*2+1, padding_idx=0)
    ...
    def _init_parameter(self):
        ...
        constant_(self.item_encoder.weight.data[:, -1], 0.0)    # item bias
