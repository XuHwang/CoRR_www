...
class Recommender(torch.nn.Module, abc.ABC):
    ...
    def add_model_specific_args(parent_parser):
        ...
        parent_parser.add_argument('--num_workers', type=int, default=8, help='num workers')
    ...
    def fit(
        ...
        **kwargs
        ...
        if len(self.logger.handlers) > 1:
            _tb_log_name = os.path.basename(self.logger.handlers[1].baseFilename).split('.')[0]
        else:
            import time
            _tb_log_name = time.strftime(f"{self.__class__.__name__}-{train_data.name}-%Y-%m-%d-%H-%M-%S.log",
                                         time.localtime())
        self.tensorboard_logger = SummaryWriter(f"tensorboard/{_tb_log_name}")
    ...
    def evaluate(self, test_data, verbose=True, **kwargs) -> Dict:
        ...
        test_loader = test_data.eval_loader(batch_size=self.config['eval_batch_size'],
                                            num_workers=self.config['num_workers'])
        output = {}
        self.load_checkpoint(os.path.join(self.config['save_path'], self.ckpt_path)) #TODO: to evaluate with saved ckpt
        self.eval()
    ...
    def training_epoch(self, nepoch):
        ...
        for loader_idx, loader in enumerate(trn_dataloaders):
            ...
            for batch_idx, batch in enumerate(loader):
                ...
                # if 'opt_idx' in inspect.getargspec(self.training_step).args:
                #     training_step_args['opt_idx'] = opt_idx
                ...
                for opt_idx, opt in enumerate(optimizers):
                    # TODO(@AngusHuang17): add grad_clip
