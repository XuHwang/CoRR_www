...
def recall(pred, target, k):
    ...
    num_pred = pred[:, :k].sum(dim=-1).float()
    output = num_pred[nonzero_mask] / count[nonzero_mask]
    return output.mean()
...
def logloss(pred, target):
    ...
    Args:
        pred(torch.FloatTensor): [B, num_items]. The prediction result of the model with bool type values.
            If the value in the j-th column is `True`, the j-th highest item predicted by model is right.
...
def mae(pred, target):
    ...
    .. math::
        MAE = \frac{1}{N}\sum_{i}^{N}\lvert y_i-\hat{y}_i\rvert

    Args:
        pred(torch.FloatTensor): prediction of scores.

        target(torch.FloatTensor): ground truth scores.

    Returns:
        torch.FloatTensor: a 0-dimensional tensor.
    """
    notpadnum = torch.logical_not(torch.isinf(pred)).float().sum()
    pred = torch.nan_to_num_(pred, nan=0.0, neginf=0.0)
    return torch.abs(pred-target).sum() / notpadnum
...
def mse(pred, target):
    ...
    notpadnum = torch.logical_not(torch.isinf(pred)).float().sum()
    pred = torch.nan_to_num_(pred, nan=0.0, neginf=0.0)
    return torch.square(pred - target).sum() / notpadnum
